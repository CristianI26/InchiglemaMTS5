...
namespace SQLiteDemo.ViewModels
{
    ...
    public partial class AddUpdateStudentDetailViewModel : ObservableObject
    {
        ...
        public async void AddUpdateStudent()
        {
            ...
            if (response > 0)
            {
                await Shell.Current.DisplayAlert("Informacion Estudiante", "Registro Guardado", "OK");
            }
            else
            {
                await Shell.Current.DisplayAlert("Alerta!", "Registro no se guardo correctamente", "OK");
            }
        }
